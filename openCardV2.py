...
def qlrun(scripts_name):
    # 读取青龙登录token
    with open("/ql/data/config/auth.json", 'rb') as json_file:
    ...
    if not RepoName:
        ...
        return
    # 运行拉取仓库任务
    File = os.path.exists("/ql/data/scripts/"+GitRepoHost[0]+"_"+GitRepoHost[1]+"/"+scripts_name)
    while not File:
...
def main():
    ...
    for x in rsp.json()["tree"]:
        if GitRepoHost[3] in x["path"]:
            tree.append(x["path"])
    # 查看是否有tree.json文件
    if not os.path.exists(f"/ql/data/scripts/{GitRepoHost[0]}_{GitRepoHost[1]}/tree.json"):
        with open(f"/ql/data/scripts/{GitRepoHost[0]}_{GitRepoHost[1]}/tree.json","w") as f:
            json.dump(tree,f)
        List.append(f"没有找到{GitRepoHost[0]}_{GitRepoHost[1]}/tree.json文件！将自动生成")
    # 读取上一次保存的tree.json并与当前tree进行对比
    with open(f"/ql/data/scripts/{GitRepoHost[0]}_{GitRepoHost[1]}/tree.json", 'rb') as json_file:
